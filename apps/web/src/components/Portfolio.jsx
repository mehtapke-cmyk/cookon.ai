...
const Portfolio = () => {
  ...
  const portfolioImages = [{
    ...
    src: 'https://horizons-cdn.hostinger.com/cea99a70-a833-4205-8479-be281a84663b/458084c01a6eef19fb80c2c8aadf3c4d.jpg',
    ...
    src: 'https://horizons-cdn.hostinger.com/cea99a70-a833-4205-8479-be281a84663b/8f1cc3c0e993b92351237f5700034e8a.jpg',
  ...
  return <section id="portfolio" className="py-24 px-6 bg-white overflow-hidden">
      <div className="max-w-7xl mx-auto">
        ...
        duration: 0.8
          ...
          <AnimatePresence initial={false} mode="popLayout">
            <motion.div key={currentIndex} variants={slideVariants} initial={direction === 'right' ? 'hiddenRight' : 'hiddenLeft'} animate="visible" exit="exit" className="w-full h-full absolute">
              <img src={portfolioImages[currentIndex].src} alt={portfolioImages[currentIndex].alt} className="w-full h-full object-cover shadow-2xl" loading="eager" />
          ...
          <div className="absolute top-1/2 -translate-y-1/2 w-full flex justify-between items-center px-4 z-10">
            <motion.button onClick={handlePrev} whileHover={{
            scale: 1.1
          }} whileTap={{
            scale: 0.95
            ...
            <motion.button onClick={handleNext} whileHover={{
            scale: 1.1
          }} whileTap={{
            scale: 0.95
          }} className="bg-white/50 hover:bg-white text-gray-800 p-3 shadow-md transition-all duration-300 backdrop-blur-sm">
          ...
          <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex gap-3 z-10">
            {portfolioImages.map((_, index) => <motion.button key={index} onClick={() => handleDotClick(index)} whileHover={{
            scale: 1.2
          }} whileTap={{
            scale: 0.9
          }} className="transition-all duration-300">
                {currentIndex === index ? <CircleDot size={14} className="text-white" /> : <Circle size={14} className="text-white/60 hover:text-white" />}
              </motion.button>)}
          </div>
        </motion.div>

        <div className="hidden">
          {portfolioImages.map((image, index) => <img key={index} src={image.src} alt="" />)}
        </div>
      </div>
    </section>;
};
